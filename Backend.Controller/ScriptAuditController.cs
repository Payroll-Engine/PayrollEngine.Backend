...
public class ScriptAuditController : Api.Controller.ScriptAuditController
{
    ...
    public async Task<ActionResult> QueryScriptAuditsAsync(int tenantId, int scriptId, [FromQuery] Query query)
    {
        // tenant check
    ...
    public async Task<ActionResult<ApiObject.ScriptAudit>> GetScriptAuditAsync(int tenantId, int scriptId, int auditId)
    {
        // tenant check
        var tenantResult = VerifyTenant(tenantId);
        if (tenantResult != null)
