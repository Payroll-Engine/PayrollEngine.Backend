...
-- VERSION_SET          -- generated by Generate-DbUpdate.ps1
...
-- VERSION_CHECK  (generated by Generate-DbUpdate.ps1)
SET XACT_ABORT ON
...
GO
USE [PayrollEngine];
-- schema changes (DROP INDEX, ALTER TABLE, DROP/CREATE PROCEDURE ...)
...
| `Compare-DbScript.ps1`   | Diff two formatted scripts, generate delta SQL with ALTER TABLE    |
| `Generate-DbUpdate.ps1`  | Full pipeline: reads `DbVersion.json`, diffs history vs. current, writes complete `Update-Model.sql` |
...
# 1. Generate Update-Model.sql (reads DbVersion.json, diffs history vs. current)
..\..\devops\scripts\Generate-DbUpdate.ps1

# 2. Review Update-Model.sql
#    ALTER TABLE statements are generated automatically.
#    Check for any remaining '-- TODO' comments (constraint/index-only changes).

# 3. After release: snapshot all scripts into History
...
Use `-DryRun` to preview what `Generate-DbUpdate.ps1` would write, and `-KeepTemp` to retain the intermediate formatted files for inspection.
...
Database\
  ...
  DbVersion.json           ← version config for Generate-DbUpdate.ps1
