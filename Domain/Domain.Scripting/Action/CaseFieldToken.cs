...
/// <summary>
/// Action token for case field
/// </summary>
internal sealed class CaseFieldToken : TokenBase
{
    ...
    internal CaseFieldToken(string action, int startIndex, string @namespace = null) : 
    ...
    protected override TokenResultData EvaluateToken(TokenParseData parseData)
    {
        ...
        switch (property)
        {
            case FieldProperty.Value:
                ...
                {
                    return new TokenResultData(parseData, $"{nameof(CaseChangeFunction.SetValue)}(\"{caseFieldName}\", ");
                }
                // get case field value
                return new(parseData, $"new {nameof(ActionValue)}({nameof(CaseChangeFunction.GetValue)}(\"{caseFieldName}\"))");
            case FieldProperty.Start:
                // set case field start date
                if (!string.IsNullOrWhiteSpace(parseData.PostCode))
                {
                    return new TokenResultData(parseData, $"{nameof(CaseChangeFunction.SetStart)}(\"{caseFieldName}\", ");
                }
                // get case field start date
                return new(parseData, $"new {nameof(ActionValue)}({nameof(CaseChangeFunction.GetStart)}(\"{caseFieldName}\"))");
            case FieldProperty.End:
                // set case field end date
                if (!string.IsNullOrWhiteSpace(parseData.PostCode))
                {
                    return new TokenResultData(parseData, $"{nameof(CaseChangeFunction.SetEnd)}(\"{caseFieldName}\", ");
                }
